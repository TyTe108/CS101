...
void deleteFront(List L){
  if (length(L)==0){
    printf("Calling deleteFront(List L) on an empty List\n");
    return;
  }
...
void delete(List L){
  ...
  if(deleteThis == L->head){
    ...
    //Whatever cursor pointed to got deleted, thefore it's no longer pointing to anything
    L->cursor = NULL;
    L->cursorIndex = -1; 
